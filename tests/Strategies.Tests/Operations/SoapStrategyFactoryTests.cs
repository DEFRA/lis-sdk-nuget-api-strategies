...
public class SoapStrategyFactoryTests
{
    ...
    public void FluentSetters_ShouldSetDefaultsAndReturnFactory()
    {
        ...
        factory.WithDefaultVerboseOutput((desc, data) => { }).ShouldBe(factory);
    }
    ...
    public void BuildSoapStrategy_ShouldCreateStrategyAndAttachAllDefaults()
    {
        ...
        typeof(SoapStrategy<TestService>).GetProperty("TargetDescription", flags)?.GetValue(soapStrategy).ShouldBe("Sample Soap Api");
        typeof(SoapStrategy<TestService>).GetProperty("BaseUrl", flags)?.GetValue(soapStrategy).ShouldBe("https://example.com/soap");
        typeof(SoapStrategy<TestService>).GetProperty("ServiceUrl", flags)?.GetValue(soapStrategy).ShouldBe("service");
        typeof(SoapStrategy<TestService>).GetProperty("SoapAction", flags)?.GetValue(soapStrategy).ShouldBe("urn:action");
        typeof(SoapStrategy<TestService>).GetProperty("MediaType", flags)?.GetValue(soapStrategy).ShouldBe("text/xml");
        typeof(SoapStrategy<TestService>).GetProperty("IncludeXmlDeclaration", flags)?.GetValue(soapStrategy).ShouldBe(true);

        var verboseAction = (Action<string, string?>?)typeof(SoapStrategy<TestService>).GetProperty("VerboseOutputAction", flags)?.GetValue(soapStrategy);

...
public class EntityRulesBuilderTests
{
    ...
    public void Add_WithPredicateFunction_AddsPredicateRuleAndReturnsSelf()
    {
        ...
        builder.Predicates[0].Description.ShouldBe("Must have ID 123");
        builder.Predicates[0].ErrorMessage.ShouldBe("Invalid ID");
        builder.Predicates[0].Predicate(new TestEntity { Id = "123" }).ShouldBeTrue();
        builder.Predicates[0].Predicate(new TestEntity { Id = "456" }).ShouldBeFalse();
    }

...
{
    private const string TestSchemaEmbeddedResource =
        "Defra.Livestock.Sdk.Api.Strategies.Tests.TestFramework.TestData.Soap.Schemas.EmbeddedTestSchema.xsd";

    private static readonly string TestSchemaFilePath =
        Path.Combine(AppContext.BaseDirectory, "TestFramework/TestData/Soap/Schemas/FileTestSchema.xsd");
    ...
    public void FluentSetters_WithValidInputs_ShouldReturnStrategy()
    {
        ...
        strategy.WithSchemas(_ => { }).ShouldBe(strategy);
    }
    ...
    public async Task Execute_WhenRequestValidationFails_ShouldThrowRequestValidationExceptionAndLogWarning()
    {
        ...
        strategy
            .WithRequestValidation(() =>
                Task.FromResult(
                    new RequestValidationResult([new RequestValidationFailure("Name", "Name is required")])));
    ...
    [Fact]
    public async Task
        Execute_WhenPreTransformSchemaValidationEnabledWithoutSchemas_ShouldThrowInvalidOperationException()
    {
        // Arrange
        var strategy = new SoapStrategy<TestService>(soapHttpClient);
    ...
    public async Task Execute_WithValidExecutionFlow_ShouldCallAllCallbacksInOrderAndReturnResult()
    {
        ...
        var soapResponse = new SoapResponse { HasContent = true, HasBody = true, BodyContent = responseXml, };
    ...
    public async Task Execute_WithPayloadObject_ShouldSerializeAndExecute()
    {
        ...
        soapHttpClient.PostAsync(
                "service",
                Arg.Do<XElement>(xml => capturedPayload = xml),
        ...
        capturedPayload.ShouldSatisfyAllConditions(
            x => x.Element("Id")?.Value.ShouldBe("123"),
            x => x.Element("Name")?.Value.ShouldBe("DirectPayload"));
    ...
    public async Task Execute_WithPayloadFactory_ShouldSerializeAndExecute()
    {
        ...
        capturedPayload.ShouldSatisfyAllConditions(
            x => x.Element("Id")?.Value.ShouldBe("456"),
            x => x.Element("Name")?.Value.ShouldBe("FactoryPayload"));
    ...
    public async Task Execute_WithPayloadTransformer_ShouldTransformPayloadBeforeSending()
    {
        ...
        strategy
            .WithPayload(() => new XElement("OriginalRequest", new XElement("Value", "original-value")))
            .WithPayloadTransformer(raw => new XElement(
                "TransformedRequest",
                new XElement("TransformedValue", raw?.Element("Value")?.Value?.ToUpperInvariant())));
        ...
        capturedPayload.ShouldSatisfyAllConditions(
            x => x.Name.LocalName.ShouldBe("TransformedRequest"),
            x => x.Element("TransformedValue")?.Value.ShouldBe("ORIGINAL-VALUE"));
    ...
    public async Task Execute_WithResponseTransformer_ShouldTransformResponseBeforeReturning()
    {
        ...
        strategy
            .WithResponseTransformer(raw => raw?.Element("NestedData"));
        ...
        result.ShouldSatisfyAllConditions(
            x => x.Name.LocalName.ShouldBe("NestedData"),
            x => x.Element("Message")?.Value.ShouldBe("TransformedSuccess"));
    ...
    public async Task ExecuteGeneric_WithResponseTransformer_ShouldTransformResponseBeforeDeserializing()
    {
        ...
        strategy
            .WithResponseTransformer(raw => raw?.Element("TestEntity"));
        ...
        var soapResponse = new SoapResponse { HasContent = true, HasBody = true, BodyContent = envelopeXml, };
        ...
        result.ShouldSatisfyAllConditions(
            x => x.Id.ShouldBe("555"),
            x => x.Name.ShouldBe("TransformedFromEnvelope"));
    ...
    public async Task ExecuteAndTransform_WithResponseTransformer_ShouldTransformResponseBeforeCustomMapping()
    {
        ...
        // Act
        var result =
            await strategy.ExecuteAndTransform(xml => new TestResult { MappedName = xml.Element("Name")?.Value! });
    ...
    public async Task ExecuteGeneric_WithValidExecutionFlow_ShouldDeserializeResult()
    {
        ...
        result.ShouldSatisfyAllConditions(
            x => x.Id.ShouldBe("999"),
            x => x.Name.ShouldBe("Deserialized"));
    }
    ...
    public async Task ExecuteAndTransform_WithXElementTransform_ShouldReturnTransformed()
    {
        ...
        // Act
        var result = await strategy.ExecuteAndTransform(xml =>
            new TestResult { MappedName = xml.Element("Value")?.Value ?? string.Empty });
    ...
    public async Task ExecuteWithoutResponse_WithSuccessfulCall_ShouldCompleteAndLog()
    {
        ...
        var soapResponse = new SoapResponse { HasContent = false, HasBody = false, BodyContent = null, };
    ...
    public async Task Execute_WhenResponseBodyContentRequiredButMissing_ShouldThrowSoapResponseException()
    {
        ...
        var soapResponse = new SoapResponse { HasContent = true, HasBody = true, BodyContent = null, };
    ...
    public async Task ExecuteAndTransform_WhenTResponseIsXElement_ShouldThrowInvalidOperationException()
    {
        ...
        var ex = await Should.ThrowAsync<InvalidOperationException>(() =>
            strategy.ExecuteAndTransform<XElement, TestResult>(_ => new TestResult()));
    ...
    [Fact]
    public async Task
        ExecuteAndTransform_WhenTResponseIsXElementWithNestedDeserializer_ShouldThrowInvalidOperationException()
    {
        // Arrange
        var strategy = new SoapStrategy<TestService>(soapHttpClient);
        ...
        // Act & Assert
        var ex = await Should.ThrowAsync<InvalidOperationException>(() =>
            strategy.ExecuteAndTransform<XElement, TestResult>((_, _) => new TestResult()));
    ...
    public async Task Execute_WithVerboseOutputAndPayloadAndResponseTransformers_EmitsVerboseLogs()
    {
        ...
        verboseOutputs.ShouldSatisfyAllConditions(
            x => x.ShouldContain(o => o.Description == "Request Body Untransformed:"),
            x => x.ShouldContain(o => o.Description == "Request Body Transformed:"),
            x => x.ShouldContain(o => o.Description == "Response Body Transformed:"));
    }
    ...
    private void ConfigureValidStrategyWithPayloadFactory<TRequest>(
        SoapStrategy<TestService> strategy,
        Func<TRequest> payloadFactory)
    {
        strategy
            .WithLogger(logger)
            .WithCancellationToken(CancellationToken.None)

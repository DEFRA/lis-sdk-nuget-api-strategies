...
{
    private const string EmbeddedSchemaResourceName =
        "Defra.Livestock.Sdk.Api.Strategies.Tests.TestFramework.Data.Soap.Schemas.EmbeddedTestSchema.xsd";

    private const string EmbeddedReferencingSchemaResourceName =
        "Defra.Livestock.Sdk.Api.Strategies.Tests.TestFramework.Data.Soap.Schemas.EmbeddedReferencingSchema.xsd";

    private static readonly string FileSchemaFilePath = Path.Combine(
        AppContext.BaseDirectory,
        "TestFramework/Data/Soap/Schemas/FileTestSchema.xsd");

    private static readonly string FileReferencingSchemaFilePath = Path.Combine(
        AppContext.BaseDirectory,
        "TestFramework/Data/Soap/Schemas/FileReferencingSchema.xsd");
...
#pragma warning disable CS8604 // Possible null reference argument.
        Should.Throw<ArgumentNullException>(() => builder.Add(null as string[]));
#pragma warning restore CS8604 // Possible null reference argument.
    ...
    public void Add_WithValidSchemaFile_ShouldBuildSchemaSet()
    {
        ...
        schemaSet.ShouldSatisfyAllConditions(
            x => x.IsCompiled.ShouldBeTrue(),
    ...
    public void Add_WithNullAssembly_ShouldThrowArgumentNullException()
    {
        ...
        // Act & Assert
        Should.Throw<ArgumentNullException>(() => { builder.Add((Assembly)null!, "resource"); });
    }
    ...
    public void Add_WithNonExistentResourceNamespace_ShouldThrowSoapSchemaException()
    {
        ...
        // Act & Assert
        var ex = Should.Throw<SoapSchemaException>(() =>
            builder.Add(Assembly.GetExecutingAssembly(), "NonExistent.Resource.xsd"));
    ...
    {
        // Arrange
        var resolver = new SoapSchemaXmlResolver { Credentials = System.Net.CredentialCache.DefaultCredentials };
    ...
    public void SoapSchemaXmlResolver_ShouldHandleResourceRegistration()
    {
        ...
        // Act
        var uri = resolver.ResolveUri(
            null,
            $"embedded://Defra.Livestock.Sdk.Api.Strategies.Tests/{EmbeddedSchemaResourceName}");
    ...
    [Fact]
    public void Add_WithFileSchemaReferencingAnotherFileSchema_BuildsSuccessfully()
    {
        // Arrange
        var builder = new SoapSchemaBuilder();

        // Act
        var returnedBuilder = builder.Add(FileReferencingSchemaFilePath);
        var schemaSet = builder.Build();
    ...
    [Fact]
    public void Add_WithEmbeddedSchemaReferencingAnotherEmbeddedSchema_BuildsSuccessfully()
    {
        // Arrange
        var builder = new SoapSchemaBuilder();

        // Act
        var returnedBuilder = builder.Add(Assembly.GetExecutingAssembly(), EmbeddedReferencingSchemaResourceName);
        var schemaSet = builder.Build();
    ...
    [Fact]
    public void SoapSchemaXmlResolver_ResolveAndGetEntity_ForReferencedFileSchema_ReturnsStream()
    {
        // Arrange
        var resolver = new SoapSchemaXmlResolver();
        resolver.RegisterFile(FileReferencingSchemaFilePath);
        var baseUri = new Uri(FileReferencingSchemaFilePath);

        // Act
        var resolvedUri = resolver.ResolveUri(baseUri, "FileTestSchema.xsd");
        var entity = resolver.GetEntity(resolvedUri, null, typeof(Stream));

        // Assert
        entity.ShouldNotBeNull();
        entity.ShouldBeAssignableTo<Stream>();
        resolvedUri.ShouldNotBeNull();

        resolvedUri.IsFile.ShouldBeTrue();

        ((Stream)entity).Dispose();
    }

    [Fact]
    public void SoapSchemaXmlResolver_ResolveAndGetEntity_ForReferencedEmbeddedSchema_ReturnsStream()
    {
        // Arrange
        var resolver = new SoapSchemaXmlResolver();

        resolver.RegisterResource(Assembly.GetExecutingAssembly(), EmbeddedReferencingSchemaResourceName);

        var baseUri =
            new Uri(
                $"embedded://{Assembly.GetExecutingAssembly().GetName().Name}/{EmbeddedReferencingSchemaResourceName}");

        // Act
        var resolvedUri = resolver.ResolveUri(baseUri, "EmbeddedTestSchema.xsd");
        var entity = resolver.GetEntity(resolvedUri, null, typeof(Stream));
        ...
        resolvedUri.Scheme.ShouldBe("embedded");
